...
The project is organized as a notebook-based workflow. Each notebook has one main responsibility, and later notebooks depend on files created by earlier notebooks.

1. `notebooks/01_eda_experiment_validation.ipynb` explores the raw Hillstrom dataset, checks treatment group sizes, validates experiment balance, and summarizes visit, conversion, and spend outcomes by campaign group.

2. `notebooks/02_feature_matrix.ipynb` creates the model-ready data files. This notebook is the source of truth for preprocessing, feature selection, treatment encoding, outcome selection, and train/test splitting. Its outputs are saved to `data/processed/`.

3. `notebooks/03_response_baseline.ipynb` builds a traditional supervised response model. This model predicts who is likely to visit or convert, but it does not directly estimate treatment effect.

4. `notebooks/04_t_learner.ipynb` through `notebooks/09_uplift_tree.ipynb` each implement one uplift or causal modeling approach. Each model notebook loads the same processed train/test files from `data/processed/`, trains its model, and saves predictions to `outputs/predictions/`.

5. `notebooks/10_evaluation_qini_auuc.ipynb` compares the saved model predictions using uplift-specific evaluation methods such as uplift-by-decile tables, Qini curves, AUUC, and top-decile lift.

6. `notebooks/11_roi_targeting_policy.ipynb` turns uplift scores into business decisions by comparing targeting strategies such as sending to everyone, sending to no one, targeting by response probability, targeting by uplift, and targeting by expected incremental profit.

7. `notebooks/12_robustness_sensitivity.ipynb` checks whether the main conclusions are stable across different assumptions, random seeds, train/test splits, treatment definitions, and ROI inputs.

The `src/` folder is intentionally small. It only stores shared helper code used by multiple notebooks, such as project paths, uplift metrics, plotting helpers, and ROI functions. Model-specific logic stays inside the model notebooks.

The main workflow is:

```text
Raw data
  -> 01 EDA and experiment validation
  -> 02 feature matrix creation
  -> 03-09 model notebooks
  -> 10 uplift evaluation
  -> 11 ROI targeting policy
...
Stores input data for the project.

```text
data/raw/
```

Stores the original Hillstrom dataset. The raw file should stay unchanged.

```text
data/processed/
```

Stores model-ready files created by `notebooks/02_feature_matrix.ipynb`, including train/test feature matrices, outcomes, treatment indicators, segment labels, and metadata.

```text
notebooks/
```

Stores the main analysis workflow. Each notebook has one clear purpose: EDA, feature matrix creation, one modeling method, evaluation, ROI, or robustness.

```text
src/
```

Stores small reusable helper files used across notebooks. This includes shared paths and constants in `config.py`, uplift evaluation helpers in `metrics.py`, plot-saving helpers in `plotting.py`, and ROI functions in `roi.py`.

```text
outputs/
```

Stores files generated by notebooks.
...
Stores saved prediction files from each model notebook. The evaluation and ROI notebooks should load model predictions from this folder.

```text
outputs/model_results/
```

Stores model-level evaluation results, such as AUUC scores, Qini scores, top-decile lift, ROI comparisons, and final model rankings.

```text
outputs/figures/
```

Stores generated plots and charts, such as EDA charts, treatment balance plots, Qini curves, uplift curves, model comparison plots, and ROI visuals.

```text
paper/
```

Stores final paper-related files, such as report drafts, final report files, LaTeX or Word documents, and selected figures for the report.

```text
presentation/
```

Stores final presentation-related files, such as slides, presentation scripts, and selected charts for the final presentation.
